...
FEATURE_COLUMNS = [
    ...
    "video length", "speech_type", "speech_sentiment"
]
...
# Function to extract sentiment from audio
from speechbrain.pretrained import EncoderClassifier
import os
from sound_analysis import extract_audio_from_video

def extract_speech_sentiment(video_path, model_dir="./pretrained_models/emotion-recognition"):
    try:
        ...
        if speech_present:
            ...
            le = joblib.load(os.path.join(model_dir, "emotion_label_encoder.pkl"))
            ...
            os.remove(audio_path)
            return "music", f"music (tempo: {round(float(tempo), 2)} BPM)"

    except Exception as e:
        print("❌ Error in speech sentiment analysis:", e)
        return np.nan, None
...
def process_video(path, filename, mode='sampled'):
    ...
    try:
        ...
        duration = np.nan
    speech_type, speech_sentiment = extract_speech_sentiment(full_path)
    print("Detected Speech Sentiment:", speech_sentiment)
    ...
    return [
        ...
        duration, speech_type, speech_sentiment
    ]
